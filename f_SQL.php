...
function ControlSQL_stk($post,$tablenum,$over)
{
	...
	$worktime = $date." ".$time;
	//社員確認
	$shain = $_SESSION['SHAID'];
        
        //------------------------//
	//          処理          //
	//------------------------//
	$control_SQL = "SELECT COUNT(*) AS count FROM ".$tableName." WHERE SHAID = '$shain' AND SHUTIME = '$worktime';";
 ...
 * 
 * @param $post			入力内容
 * @param $tablenum		使用テーブル
 * 
 * 戻り値　insert_SQL	登録SQL
 */
function InsertSQL_stk($post,$tablenum,$over)
{
	...
	//          処理          //
	//------------------------//
	$insert_SQL = "INSERT INTO ".$tableName." (SHAID,SHUTIME,AUTOSHUDATE,";
	...
	$insert_SQL = substr($insert_SQL,0,-1);
	//ここから値
	$insert_SQL .= ")VALUES('$shain','$worktime',CURRENT_TIMESTAMP, ";
	...
	if($update_value != '')
	{
		for( $i = 0 ; $i < count($update_value_array) ; $i++)
		{
			...
			if( $pos == false )
			{
				$insert_SQL .= $update_value_array[$i].",";
			}
			else
			{
				$outvalue = "";
 ...
 * 
 * @param $post			入力内容
 * @param $tablenum		更新テーブル 
 * @param $stkid	更新コード
 * 
 * 戻り値　update_SQL	更新SQL
 */
function UpdateSQL_stk($post,$tablenum,$stkid)
{
	//------------------------//
	//        初期設定        //
	//------------------------//
	...
	$update_SQL .= "UPDATE ".$tableName." SET TAITIME = '$worktime', AUTOTAIDATE = CURRENT_TIMESTAMP, ";
	...
	if($update_column != '')
	{
		for( $i = 0 ; $i < count($update_column_array) ; $i++)
		{
			...
			{
				$update_SQL .= ' '.$columnName.' = ';
				$update_SQL .= $update_value_array[$i].",";
			}
			else
			{
				$outvalue = "";
	...
	//条件を指定
	$update_SQL .= " WHERE ".$tablenum."ID = $stkid";
	$update_SQL .= ";";
