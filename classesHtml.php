 ...
 * 
 */
class MitsumoriPrintPage extends BasePage
{
	...
	function makeBoxContentMain()
	{
		$print = '';
		//印刷用
		$print .= "<div class='printpage'>";
			//日付分解
			 $Mitsumoribi = explode('/', $this->Mitsumoribi);
			//配列になっていなかったら
			if (count($Mitsumoribi) != 3 )
			{
				$Mitsumoribi = explode('-', $this->Mitsumoribi);
			}
			/*日付、見積No*/
			$print .= "<div class='dateBox'>";
				$print .= "<table class='date'>";
				$print .= "<tr>";
				$print .= "<td>日付:</td>";
				$print .= "<td>$Mitsumoribi[0]年$Mitsumoribi[1]月$Mitsumoribi[2]日</td>";
				$print .= "</tr>";
				$print .= "<tr>";
				$print .= "<td>見積No:</td>";
				$print .= "<td>$this->Code</td>";
				$print .= "</tr>";
				$print .= "</table>";
			$print .= "</div>";
			...
			$print .= "<p class='kigyo'>";
			//$print .= "マーキュリーソフト株式会社";
			$print .= "$this->MitsumoriAte";
			$print .= "</p>";

			/*下線*/
			$print .= "<hr align='left'color='black' class='border'>";
			/*固定部分*/
			$print .= "<div class='fix'>";
			$print .= "御中";
			$print .= "</div>";

			//$print .= "<p class='tanto'>ご担当 : ＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸ </p>";
			$print .= "<p class='tanto'>ご担当 : $this->Kokyakutanto 　様</p>";

			/*固定部分*/
			//$print .= "<div class='fix2'>";
			//$print .= "様";
			//$print .= "</div>";

			//$print .= "<p class='kenmei'>件名: ＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸＸ";
			$print .= "<p class='kenmei'>件名: $this->Kenmei";
			/*下線*/
			$print .= "<hr align='left'color='black' class='border'>";
			$print .= "</div>";

			//ハンコ画像
			$print .= "<div class='hanko'>";
			
				$print .= "<img src='./image/sample.png' class='png'>";
			$print .= "</div>";

			//金額、有効期限、自社情報、印鑑部分の枠
			$print .= "<div class='box'>";

			$print .= "<div class='kingakuBox'>";
			$print .= "下記の通りお見積り申し上げます。";
			$print .= "</br>";
			$print .= "</br>";
			$print .= "</br>";
	...
	{
		$html = $this->makePrintPage($this->prContainer->pbInputContent,$this->prContainer->pbPageSetting, $this->prContainer->pbFormIni, $this->prContainer->pbFileName);
		$html .= '<div class = "print"><input type="button" value="印刷" id="print" class="print"  onClick="window.print()"></div>';
		return $html;
	}
}
...
class InsertPage extends BasePage
{
	...
	function makeBoxContentMain()
	{
		...
		$send = '<form name ="insert" action="main.php?'.$this->prContainer->pbFileName.'=" method="post" enctype="multipart/form-data" 
		...
		if( isPermission($this->prContainer->pbFileName) )
		{
			...
			if($this->prContainer->pbPageSetting['form_type'] !== '2')
			{
				...
				$html .= '<div class = "center">
					<input type="submit" name = "insert" value = "登録" class="free">';
				...
				{	
					$html .= '<input type="submit" name = "print" value="印刷" class="free"  >';
				}
				$html .='</div>';
			}
...
class InsertCheckPage extends InsertPage
{
	...
	function makeBoxContentMain()
	{
		...
		$errorinfo = existCheck($this->prContainer->pbInputContent,'',$this->prMainTable,1);
	...
	*/
	...
	function makeBoxContentBottom()
	{
		...
		if( $is_permission ){
			...
			if($this->prContainer->pbPageSetting['form_type'] !== '2')
			{
                            if($_SESSION['filename'] === "USERMASTER_3")
                            {
                                $html .= '<div class = "pad">
				...
				$html .= '<div class = "pad">
					<input type="submit" name = "kousinn" value = "更新" class="free" onclick="document.getElementByName(\'edit\').action=\'main.php?'.$this->prContainer->pbFileName.'_button=\'" >
					<input type="submit" name = "delete" value = "削除" class="free" onClick = "ischeckpass = false;">';
                            }
				if($this->prContainer->pbFileName == 'MITSUMORIINFO_1' || $this->prContainer->pbFileName == 'SEIKYUINFO_1')
				{	
					$html .= '<input type="submit" name = "print" value="印刷" class="free" onclick="document.getElementByName(\'edit\').action=\'main.php?MITSUMORIPRINT_5_button=\'" >';
				}
...
class EditCheckPage extends EditPage
{
	...
	function makeBoxContentMain()
	{
		...
		if($isexist)
		{
			...
			$errorinfo = existCheck($this->prContainer->pbInputContent,$this->prContainer->pbListId,$main_table,2);
...
class SeikyuPrintPage extends MitsumoriPrintPage
{
